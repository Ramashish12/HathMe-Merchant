...
import code.utils.UserInfoUtils;
...
public abstract class CallActivity extends AppCompatActivity {
    ...
    private String remoteProfileUrl = "";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Log.i(MyApplication.TAG, "[CallActivity] onCreate()");
        ...
        if (mDoEnd) {
            Log.i(MyApplication.TAG, "[CallActivity] init() => (mDoEnd == true)");
            end();
            return;
        }
...
//        checkAuthentication();
    ...
    private void init() {
        ...
        remoteUserName = intent.getStringExtra(CallService.EXTRA_REMOTE_NICKNAME_OR_USER_ID);
        remoteProfileUrl = intent.getStringExtra(CallService.EXTRA_PROFILE_URL);
        ...
        Log.i(MyApplication.TAG, "[CallActivity] init() => (mState: " + mState + ", mCallId: " + mCallId + ", mIsVideoCall: " + mIsVideoCall
    ...
    protected void onNewIntent(Intent intent) {
        ...
        Log.i(MyApplication.TAG, "[CallActivity] onNewIntent()");

        mDoEnd = intent.getBooleanExtra(CallService.EXTRA_DO_END, false);
        if (mDoEnd) {
            Log.i(MyApplication.TAG, "[CallActivity] onNewIntent() => (mDoEnd == true)");
            end();
        }
    }
    ...
    protected void setViews() {
        ...
        mImageViewAudioOff.setOnClickListener(view -> {
            if (mDirectCall != null) {
                if (mIsAudioEnabled) {
                    Log.i(MyApplication.TAG, "[CallActivity] mute()");
                    mDirectCall.muteMicrophone();
                    mIsAudioEnabled = false;
                    mImageViewAudioOff.setSelected(true);
                } else {
                    Log.i(MyApplication.TAG, "[CallActivity] unmute()");
    ...
    protected void setListener(DirectCall call) {
        Log.i(MyApplication.TAG, "[CallActivity] setListener()");

        if (call != null) {
            call.setListener(new DirectCallListener() {
                @Override
                public void onConnected(DirectCall call) {
                    Log.i(MyApplication.TAG, "[CallActivity] onConnected()");
                    setState(STATE.STATE_CONNECTED, call);
                }

                @Override
                public void onEnded(DirectCall call) {
                    Log.i(MyApplication.TAG, "[CallActivity] onEnded()");
                ...
                public void onRemoteVideoSettingsChanged(DirectCall call) {
                    Log.i(MyApplication.TAG, "[CallActivity] onRemoteVideoSettingsChanged()");
                }

                @Override
                public void onLocalVideoSettingsChanged(DirectCall call) {
                    Log.i(MyApplication.TAG, "[CallActivity] onLocalVideoSettingsChanged()");
                ...
                public void onRemoteAudioSettingsChanged(DirectCall call) {
                    Log.i(MyApplication.TAG, "[CallActivity] onRemoteAudioSettingsChanged()");
                ...
                public void onAudioDeviceChanged(DirectCall call, AudioDevice currentAudioDevice, Set<AudioDevice> availableAudioDevices) {
                    Log.i(MyApplication.TAG, "[CallActivity] onAudioDeviceChanged(currentAudioDevice: " + currentAudioDevice + ", availableAudioDevices: " + availableAudioDevices + ")");
    ...
    protected void setInfo(DirectCall call, String status) {
        ...
        if (remoteUser != null) {
            ...
            UserInfoUtils.setProfileImage(mContext, remoteProfileUrl, mImageViewProfile);
        }


//        mTextViewUserId.setText(getRemoteNicknameOrUserId(call));
        mTextViewUserId.setText(remoteUserName);
    ...
    private void setRemoteMuteInfo(DirectCall call) {
        if (call != null && !call.isRemoteAudioEnabled() && call.getRemoteUser() != null) {
            mTextViewRemoteMute.setText(getString(R.string.calls_muted_this_call,remoteUserName));
            mLinearLayoutRemoteMute.setVisibility(View.VISIBLE);
        } else {
            mLinearLayoutRemoteMute.setVisibility(View.GONE);
        }
    ...
    private void end() {
        if (mDirectCall != null) {
            ...
            Log.i(MyApplication.TAG, "[CallActivity] end()");

            if (mState == STATE.STATE_ENDING || mState == STATE.STATE_ENDED) {
                Log.i(MyApplication.TAG, "[CallActivity] Already ending call.");
                return;
            }
            ...
            Log.i(MyApplication.TAG, "[CallActivity] end() => (mDirectCall == null)");
            finishWithEnding("(mDirectCall == null)");
        }
    }

    protected void finishWithEnding(String log) {
        Log.i(MyApplication.TAG, "[CallActivity] finishWithEnding(" + log + ")");
        ...
        if (mEndingTimer == null) {
            ...
            mEndingTimer.schedule(new TimerTask() {
                ...
                public void run() {
                    runOnUiThread(() -> {
                        Log.i(MyApplication.TAG, "[CallActivity] finish()");
                        finish();
    ...
    protected void onDestroy() {
        super.onDestroy();
        Log.i(MyApplication.TAG, "[CallActivity] onDestroy()");
    ...
    private ServiceConnection mCallServiceConnection = new ServiceConnection() {
        ...
        public void onServiceConnected(ComponentName componentName, IBinder iBinder) {
            Log.i(MyApplication.TAG, "[CallActivity] onServiceConnected()");
        ...
        public void onServiceDisconnected(ComponentName componentName) {
            Log.i(MyApplication.TAG, "[CallActivity] onServiceDisconnected()");
    ...
    private void bindCallService() {
        Log.i(MyApplication.TAG, "[CallActivity] bindCallService()");
    ...
    private void unbindCallService() {
        Log.i(MyApplication.TAG, "[CallActivity] unbindCallService()");
    ...
    private void stopCallService() {
        Log.i(MyApplication.TAG, "[CallActivity] stopCallService()");
    ...
    protected void updateCallService() {
        if (mCallService != null) {
            Log.i(MyApplication.TAG, "[CallActivity] updateCallService()");


            CallService.ServiceData serviceData = new CallService.ServiceData();
            serviceData.isHeadsUpNotification = false;
            serviceData.remoteNicknameOrUserId = remoteUserName;/*getRemoteNicknameOrUserId(mDirectCall);*/
