...
public class VideoCallActivity extends CallActivity {
    ...
    protected void initViews() {
        super.initViews();
        Log.i(MyApplication.TAG, "[VideoCallActivity] initViews()");
    ...
    protected void setViews() {
        ...
        mImageViewCameraSwitch.setOnClickListener(view -> {
            if (mDirectCall != null) {
                mDirectCall.switchCamera(e -> {
                    if (e != null) {
                        Log.i(MyApplication.TAG, "[VideoCallActivity] switchCamera(e: " + e.getMessage() + ")");
                    }
                });
            }
        });
        ...
        mImageViewVideoOff.setOnClickListener(view -> {
            if (mDirectCall != null) {
                if (mIsVideoEnabled) {
                    Log.i(MyApplication.TAG, "[VideoCallActivity] stopVideo()");
                    mDirectCall.stopVideo();
                    mIsVideoEnabled = false;
                    mImageViewVideoOff.setSelected(true);
                } else {
                    Log.i(MyApplication.TAG, "[VideoCallActivity] startVideo()");
    ...
    protected void setLocalVideoSettings(DirectCall call) {
        ...
        Log.i(MyApplication.TAG, "[VideoCallActivity] setLocalVideoSettings() => isLocalVideoEnabled(): " + mIsVideoEnabled);
        mImageViewVideoOff.setSelected(!mIsVideoEnabled);
    }
    ...
    protected void startCall(boolean amICallee) {
        ...
        if (amICallee) {
            Log.i(MyApplication.TAG, "[VideoCallActivity] accept()");
            if (mDirectCall != null) {
                mDirectCall.accept(new AcceptParams().setCallOptions(callOptions));
            }
        } else {
            Log.i(MyApplication.TAG, "[VideoCallActivity] dial()");
            ...
            mDirectCall = SendBirdCall.dial(new DialParams(mCalleeIdToDial)
                    .setVideoCall(mIsVideoCall)
                    .setCallOptions(callOptions)
                    .setCustomItems(map), (call, e) -> {
                if (e != null) {
                    Log.i(MyApplication.TAG, "[VideoCallActivity] dial() => e: " + e.getMessage());
...
//                        ToastUtils.showToast(mContext, e.getMessage());
                ...
                Log.i(MyApplication.TAG, "[VideoCallActivity] dial() => OK");
                updateCallService();
            });
    ...
    protected void onStart() {
        super.onStart();
        Log.i(MyApplication.TAG, "[VideoCallActivity] onStart()");
    ...
    protected void onStop() {
        super.onStop();
        Log.i(MyApplication.TAG, "[VideoCallActivity] onStop()");
